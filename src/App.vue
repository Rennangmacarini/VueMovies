...
<template>
    <div class="fixed w-full h-screen bg-black">
      <div v-if="!showFullVideo" id="SideNav" class="flex z-40 items-center w-[120px] h-screen bg-black relative">
       ...
       <div>
        <div class="py-2 max-10 my-6">
            <Magnify fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
         <div class="py-2 max-10 my-6 border-b-4 border-b-red-500">
            <HomeOutline fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
        <div class="py-2 max-10 my-6 border-b-4" > 
            <TrendingUp fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
        <div class="py-2 max-10 my-6 border-b-4 ">
            <Television fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
        <div class="py-2 max-10 my-6 border-b-4 ">
            <MovieOutline fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
        <div class="py-2 max-10 my-6 border-b-4">
            <Plus fillColor="#ffff" :size="40" class="cursor-pointer" />
        </div>
       </div>
      </div>
      ...
      <div v-if="!showFullVideo" class="absolute z-20 h-[70%] left-[120] w-[100%] right-0 bottom-0 bg-gradient-to-t from-black via-black"></div>

      <div v-if="showFullVideo">
        <div @click="$event => showFullVideo = false" class="absolute z-50 p-2 m-4 bg-white bg-opacity-50 rounded-full cursor-pointer">
            <ChevronLeft fillColor = "FFFF" :size="40" />
        </div>
         <video 
                v-if="movie"
                :src="'/videos/'+movie.name+'.mp4'"
